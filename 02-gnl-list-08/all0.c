...
/*   Updated: 2023/03/04 19:28:52 by antoda-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*split_line(t_fd_lst *node, char *line)
{
	...
	i = 0;
	len = ft_strlen(line);
	printf("############ split line : len line = %i ############\n", len);
	tmp = line;
	while (tmp[i] && tmp[i] != '\n')
		i++;
	if (tmp[i] == '\n')
	{
		printf("split line : str = strsub(line, i + 1, len - (i + 1)) %i, %i\n", i+1, len - (i + 1));
		node->str = ft_strsub(line, i + 1, len - (i + 1));
		printf("split line : line = ft_strsub(line, 0, i + 1)[%i, %i]\n", 0, i+1);
...
char	*pending_line(t_fd_lst *node, char *line)
{
	...
	printf("############ pendi line : len str = %i ############\n", len);
	tmp = node->str;
	while (tmp[i] && tmp[i] != '\n')
		i++;
	if (tmp[i] == '\n')
	{
		printf("pending line : line = ft_strsub(node->str, 0, %i + 1)\n", i);
		line = ft_strsub(node->str, 0, i + 1);
		printf("pending line : node->str  = t_strsub(node->str, %i + 1, ft_strlen(node->str)(%li)\n", i, ft_strlen(node->str));
